...
namespace LE.Responces {
    class Auth {
        public static void ProcessAuthorization(ClientHandler.ioData io, ref CLIENT_STRUCT ClientObj) {
            ...
            bool ClientFound = MySql.GetClient(ref ClientObj, FuseLineKey);
            ClientObj.consoletype = Utilities.GetConsoleType(ConsoleType);
            if (!ClientFound && ClientObj.consoletype != "Error") {
                MySql.InsertClient(FuseLineKey, Utilities.BytesToString(SessionToken));
                ClientFound = MySql.GetClient(ref ClientObj, FuseLineKey);
            }
            if (!ClientFound || ClientObj.consoletype == "Error") {
                Console.WriteLine("Console type error for client Cpu: " + FuseLineKey);
            ...
            MySql.SaveClient(ClientObj, FuseLineKey);
            ...
            Log.Add(Authid, ConsoleColor.Magenta, "CPUKey:", $"{FuseLineKey}");
